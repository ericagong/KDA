...
const SingleSearchApis = () => {
  ...
  const searchSummonersHandler = async (e) => {
    ...
    console.log(`[searchSummonersHandler] summonerNames: ${summonerNames}`)
    ...
    try {
      ...
      console.log(
        `[searchSummonersHandler] res.data: ${JSON.stringify(res.data)}`,
      )
  ...
  // CHECK D2Basic 미반영 이슈 -> 해결 (useState 낡은 클로저)
  useDidMountEffect(() => {
    async function getD2BasicInfo() {
      try {
        ...
        // setTimes({...times, D2Basic: timeCalculator(start, end)})
    ...
    async function getMatchesInfo() {
      try {
        ...
        const matchInfoList = await Promise.all(
          D2MatchInfo.map(async (matchID) => {
            return await apis
              .get_depth3_match_info(matchID, D1Info.ID)
              .then((res) => res.data)
          }),
        )
...
export default SingleSearchApis
